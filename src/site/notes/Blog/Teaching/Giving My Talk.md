---
{"dg-publish":true,"dg-path":"Teaching/Giving My Talk.md","permalink":"/teaching/giving-my-talk/","created":"2026-02-20T13:28:29.632+01:00","updated":"2026-02-27T12:09:54.151+01:00"}
---
...
For example, take the problem of computing the importance of every webpage on the internet, which is roughly what Google does. At the level of a five-year-old, think of it as deciding the most popular kid in school. Instead of just counting how many friends you have, you also care about how popular those friends are; an endorsement from the most popular kid counts more than one from someone nobody knows. Everyone votes for their friends, but popular kids' votes count more. Now imagine everyone keeps voting round after round, and popularity (the weight of each vote) changes over time. However, after enough rounds, the votes magically stabilize and nobody changes their answer anymore. They have agreed on who is most popular.

A computer engineer would frame it differently: represent the web as a directed graph and rank nodes by importance, where importance is circular - a node is important if important nodes point to it. The trick is to turn this into an iterative algorithm: start with every page having equal score, then repeatedly update each page's score to be the sum of the scores of all pages pointing to it, divided by their out-degree. Repeat until convergence.

A mathematician would then observe that what you're really doing is multiplying a vector (the current importance scores) by a matrix (the normalized link structure encoding who is connected to whom and by how much) repeatedly until it stops changing. The fixed point is the final ranking. That fixed point is the eigenvector of the matrix with eigenvalue 1. Indeed, $\pi = P\pi$ is precisely the eigenvector equation: $\pi$ is the vector that is completely unchanged under the transformation $P$. Any starting vector can be written as the linear combination of eigenvectors and after repeatedly applying $P$, all components except $\pi$ decay to zero since their eigenvalues are strictly less than 1. What remains is $\pi$. It is also the stationary distribution of an ergodic Markov chain on the web graph: the long-run landing probability of a random surfer, regardless of where they start. The Perron-Frobenius and the Ergodic theorem guarantee that this distribution exists and is unique, and explain both why the initial weight of each kid don't matter and why the votes "magically" stabilize.

In this example, presenting only the mathematician's viewpoint would leave most audiences behind, including many computer engineers. The kid example, by contrast, adds connections to almost everyone's knowledge graph and helps understand and remember the problem, even if not the complete solution. That is often enough: a motivated audience will go and read the details themselves, which is the ultimate goal of most talks. 

Note that the above presentation deliberately skips a few technically important details, such as $P$ must be stochastic and diagonalizable, conditions that may not hold in practice without the damping factor. But that is forgivable in a 15 minute talk, and less in a lecture. Dwelling on these details risks losing the audience before they become motivated enough to care about them.

Take linear algebra as an another general example. Almost every concept in it has a clean geometric interpretation accessible to nearly anyone. A matrix is not just a grid of numbers, it is a transformation of space. Matrix multiplication is simply applying two transformations in sequence. The determinant measures how much the transformation stretches or squashes space; it is the area of the parallelogram formed by the column vectors in 2D. A determinant of zero means the parallelogram has collapsed to a line: the vectors are linearly dependent, information is lost, and the matrix cannot be inverted. The null space is the set of directions that get squashed to zero, and the rank is how many dimensions survive. The dot product measures the shadow of one vector onto another, capturing how much they point in the same direction, which is why it appears everywhere from similarity measures to covariance in probability. Even Singular Value Decomposition (SVD), which can seem quite abstract, is in fact just a rotation, followed by a scaling, followed by another rotation - much like prime factorization reduces any integer to its simplest components. This geometric viewpoint also makes more advanced topics that build on linear algebra, such as Markov chain theory or machine learning, more coherent and accessible. The more clusters you can link a concept to in your knowledge graph, the more robustly it is represented and the more naturally it will be recalled and applied later. 
